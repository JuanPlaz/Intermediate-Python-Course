...
def run():
    ...
    #Using HOF.
    all_platzi_workers_list = list(filter(lambda worker: worker["organization"] == "Platzi", DATA))
    all_platzi_workers_list = list(map(lambda worker: worker["name"], all_platzi_workers_list))

    #Using HOF.
    adults = list(filter(lambda worker: worker["age"] > 18 , DATA))
    adults = list(map(lambda worker: worker["name"], adults ))
    old_people = list(map(lambda worker: worker | {"old": worker["age"] > 70 }, DATA)) ##El pipe | nos sirve para sumar diccionarios.
